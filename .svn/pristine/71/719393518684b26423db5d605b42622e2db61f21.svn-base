...
public class EasemobChatGroups {
	...
	// 'Authorization: Bearer {token}'
	// * -d
	// '{"groupname":"测试群组","desc":"测试群组","public":true,"approval":true,"owner":"xiaojianguo001","maxusers":333,"members":["xiaojianguo002","xiaojianguo003"]}'
	// */
	...
	// ObjectNode getAllMemberssByGroupIdNode =
	// getAllMemberssByGroupId(chatgroupid);
	...
	// * curl示例
	// * curl -X POST
	// 'https://a1.easemob.com/easemob-playground/test1/chatgroups/1405735927133519/users/xiaojianguo002'
	// * -H 'Authorization: Bearer {token}'
	// */
	...
	// ObjectNode deleteUserFromGroupNode =
	...
	// * curl示例
	// * curl -X GET
	// 'https://a1.easemob.com/easemob-playground/test1/users/xiaojianguo002/joined_chatgroups'
	// * -H 'Authorization: Bearer {token}'
	// */
	// String username = "xiaojianguo002";
	// ObjectNode getJoinedChatgroupsForIMUserNode =
	// getJoinedChatgroupsForIMUser(username);
	...
	// ObjectNode addUserToGroupBatchNode =
	// addUsersToGroupBatch(toAddBacthChatgroupid, usernamesNode);
	...
	public static ObjectNode getGroupDetailsByChatgroupid(String[] chatgroupIDs) {
		...
		try {
			URL groupDetailsByChatgroupidUrl = HTTPClientUtils
					.getURL(Constants.APPKEY.replace("#", "/") + "/chatgroups/" + chatgroupIDs.toString());
	...
	public static ObjectNode deleteChatGroups(String chatgroupid) {
		...
		try {
			URL deleteChatGroupsUrl = HTTPClientUtils
					.getURL(Constants.APPKEY.replace("#", "/") + "/chatgroups/" + chatgroupid);
	...
	public static ObjectNode getAllMemberssByGroupId(String chatgroupid) {
		...
		try {
			URL allMemberssByGroupIdUrl = HTTPClientUtils
					.getURL(Constants.APPKEY.replace("#", "/") + "/chatgroups/" + chatgroupid + "/users");
	...
	public static ObjectNode addUserToGroup(String chatgroupid, String userName) {
		...
		try {
			URL allMemberssByGroupIdUrl = HTTPClientUtils
					.getURL(Constants.APPKEY.replace("#", "/") + "/chatgroups/" + chatgroupid + "/users/" + userName);
	...
	private static ObjectNode getJoinedChatgroupsForIMUser(String username) {
		...
		try {
			URL getJoinedChatgroupsForIMUserUrl = HTTPClientUtils
					.getURL(Constants.APPKEY.replace("#", "/") + "/users/" + username + "/joined_chatgroups");
	...
	private static ObjectNode addUsersToGroupBatch(String toAddBacthChatgroupid, ObjectNode usernames) {
		...
		try {
			URL getJoinedChatgroupsForIMUserUrl = HTTPClientUtils
					.getURL(Constants.APPKEY.replace("#", "/") + "/chatgroups/" + toAddBacthChatgroupid + "/users");
