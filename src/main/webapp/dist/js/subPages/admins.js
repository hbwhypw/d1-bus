...
Admins.prototype.add = function () {
    var _this = this;
    $("#adminsDlg").dialog("open").dialog("setTitle", "添加管理员");
    $('#adminsFm').form('clear');
    $("#is_check option:first").prop("selected", 'selected');
    _this.url = '/admins/add';
};
...
Admins.prototype.update = function () {
    ...
    if (row) {
        $("#adminsDlg").dialog("open").dialog("setTitle", "修改管理员");
        $('#adminsFm').form('load', row);
        _this.url = '/admins/update';
    } else {
...
Admins.prototype.query = function () {
    ...
    this.checkInput('nick_nameParam',this.INPUT);
...
Admins.prototype.validUserName = function() {
	$("#username").focusout(function(){
		var username = $("#username").val();
		$.post('/admins/selectByUserName', {
    		"username": username
    	}, function(data) {
    		if(0==data.code){
    			$.messager.show({
